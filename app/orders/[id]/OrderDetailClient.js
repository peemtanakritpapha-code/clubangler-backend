...
function StepTimeline({ status, steps }) {
  if (RETURN_FLOW.includes(status)) return null;
        ...
        <div style={card}>
          ...
          {inReturn && (
            <div style={{ background: o.status === "refunded" ? "#F1F3F4" : o.status === "disputed" ? "#FFF5F5" : "#FFF7ED", border: `1px solid ${o.status === "disputed" ? "#FCA5A5" : "#FED7AA"}`, borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 12.5, fontWeight: 800, color: o.status === "disputed" ? C.danger : C.ret, margin: "8px 0 4px" }}>
                {o.require_return ? "เคสขอคืนสินค้า" : "เคสพิพาท (ไกล่เกลี่ย)"} — {o.dispute_reason}
              </div>
              {o.dispute_detail && <div style={{ fontSize: 12.5, color: C.ink, lineHeight: 1.6 }}>"{o.dispute_detail}"</div>}
              ...
              {o.auto_confirmed && <div style={{ fontSize: 11, color: C.ret, marginTop: 6 }}>⏰ ระบบยืนยันรับของคืนแทนผู้ขาย (ครบกำหนด)</div>}
            </div>
          )}
